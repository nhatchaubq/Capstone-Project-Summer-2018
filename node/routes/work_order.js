...
module.exports = function (io) {
    ...
    //get workorder Inprogress
    //TienNT
    router.get("/workorderbyUserId/:id", (request, response) => {
        request
            .sql(
                " select distinct wo.*, loca.Name as 'Location',loca.Id as [LocationID]  ,t.Name as 'Team' ,ws.Name as 'Status',wc.Name as 'Category', " +
                "(select distinct e.Id,e.Name,e.Image,wd.WorkOrderID,wd.ClosedDate, ei.SerialNumber, ei.Id as [EquipmentItemID], ei.TileID as [TileID], tile.Name as [Tile], floor.Id as [FloorID], floor.Name as [FLoor] ,block.Id as [BlockID], block.Name as [Block], loca.Id as [LocationID], loca.Name as [Location] " +
                "   from WorkOrderDetail as wd join EquipmentItem as ei on wd.EquipmentItemID = ei.Id " +
                "   join Equipment as e on e.Id = ei.EquipmentID " +
                ...
                " join WorkOrderCategory as wc on wc.Id = wo.CategoryID						  " +
                " where acc.Id = @userId and(ws.Name = 'In Progress') " +
                " order by ws.Name desc " +
                "  for json path "
