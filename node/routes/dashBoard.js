...
router.get("/", (request, response) => {
  request
    .sql(
      ...
      // "(SELECT Count(WorkOrderID) FROM [WorkOrderDetail] WHERE WorkOrderID IN (SELECT Id FROM [WorkOrder] WHERE CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE woc.Name = N'Maintain')  AND WorkOrder.CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE  woc.Name = N'Working')  and convert(date, getdate()) = convert(date, ExpectingCloseDate))) as 'Dashboard.MaintainCountTodayQItem'," +
      // "(SELECT Count(WorkOrderID) FROM [WorkOrderDetail] WHERE WorkOrderID IN (SELECT Id FROM [WorkOrder] WHERE CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE woc.Name = N'Maintain') AND WorkOrder.CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE  woc.Name = N'Working')  and convert(date, dateadd(day, 1, getdate())) = convert(date, ExpectingCloseDate))) as 'Dashboard.MaintainCountTomorrowQItem'," +
      // "(SELECT Count(WorkOrderID) FROM [WorkOrderDetail] WHERE WorkOrderID IN (SELECT Id FROM [WorkOrder] WHERE CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE woc.Name = N'Working') AND WorkOrder.CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE  woc.Name = N'Working')  and convert(date, getdate()) = convert(date, ExpectingCloseDate))) as 'Dashboard.ReturnCountTodayQItem'," +
      // "(SELECT Count(WorkOrderID) FROM [WorkOrderDetail] WHERE WorkOrderID IN (SELECT Id FROM [WorkOrder] WHERE CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE woc.Name = N'Working') AND WorkOrder.CategoryID = (SELECT woc.Id FROM [WorkOrderCategory] as woc WHERE  woc.Name = N'Working') and convert(date, dateadd(day, 1, getdate())) = convert(date, ExpectingCloseDate))) as 'Dashboard.ReturnCountTomorrowQItem'," +
      // dien-change count today,tomorrow WO and item count-end
