...
router.get("/", (request, response) => {
  request
    .sql(
      ...
      "(SELECT COUNT(StatusId)  FROM [EquipmentItem] WHERE StatusId = (SELECT es.Id FROM [EquipmentStatus] as es WHERE es.Name = N'WORKING REQUESTING')) as 'PieChartData.WokingRequesting.Quantity', (SELECT es.Name FROM [EquipmentStatus] as es WHERE es.Name = N'WORKING REQUESTING') as 'PieChartData.WokingRequesting.Name'," +
      "(SELECT COUNT(StatusId)  FROM [EquipmentItem] WHERE StatusId = (SELECT es.Id FROM [EquipmentStatus] as es WHERE es.Name = N'MAINTAINANCE REQUESTING')) as 'PieChartData.MaintainanceRequesting.Quantity', (SELECT es.Name FROM [EquipmentStatus] as es WHERE es.Name = N'MAINTAINANCE REQUESTING') as 'PieChartData.MaintainanceRequesting.Name'," +
      ...
      "(select [Name] from EquipmentStatus where Id = (select es.Id from [EquipmentStatus] as es where es.Name = N'WORKING REQUESTING')) as 'Doughnut.Unavailable.Name'" +
