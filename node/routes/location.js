...
router.get("/getLocation/:accountId", (req, res) => {
  req
    .sql(
      "Select distinct l.* " +
    ...
    .param("accountId", req.params.accountId, TYPES.Int)
    .into(res);
});
router.get("/warehouse", (request, response) => {
  request
    .sql(
      "Select * From [Location] as loca where loca.Name like '%Kho%' " +
        "for json path"
    )
    .into(response);
});
