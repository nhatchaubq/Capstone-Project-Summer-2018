...
router.get("/:id", (request, response) => {
  request
    .sql(
      ...
      "(select acc.Id, acc.Username from [Account] as acc WHERE acc.RoleID = (SELECT r.Id FROM [role] as r WHERE r.Name = N'Staff' ) and acc.IsActive = N'True' )  except (select acc.Id,acc.Username from [Account] as acc  join [TeamAccount] as ta ON acc.Id = ta.AccountID where TeamID = @teamID and acc.IsActive = N'True' ) for json path"
    )
